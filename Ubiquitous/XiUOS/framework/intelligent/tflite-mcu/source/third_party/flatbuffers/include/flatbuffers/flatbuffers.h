...
class FlatBufferBuilder {
 public:
  ...
  void NotNested() {
    ...
    // not get this CHECK.
    // Ignoring this CHECK may appear to work in simple cases, but the reason
  ...
  uoffset_t EndTable(uoffset_t start) {
    // If you get this CHECK, a corresponding StartTable wasn't called.
    FLATBUFFERS_ASSERT(nested);
  ...
  template<typename T> Offset<Vector<T>> CreateVector(const T *v, size_t len) {
    // If this CHECK hits, you're specifying a template argument that is
...
inline const uint8_t *GetBufferStartFromRootPointer(const void *root) {
  ...
  // CHECK, because calling this function with bad data may cause reads
