...
// instead. Normally this would be an CHECK, but since this is "dynamically
...
// TODO(wvo): Could introduce an #ifdef that makes this into an CHECK?
...
class Builder FLATBUFFERS_FINAL_CLASS {
 public:
  ...
  size_t EndMap(size_t start) {
    ...
    // for maximum efficiency? With an CHECK to check sortedness to make sure
  ...
  void Finish() {
    // If you hit this CHECK, you likely have objects that were never included
 ...
 private:
  void Finished() const {
    // If you get this CHECK, you're attempting to get access a buffer
 ...
 private:
  ...
  size_t ScalarVector(const T *elems, size_t len, bool fixed) {
    ...
    // If you get this CHECK, you're trying to write a vector with a size
    ...
    for (size_t i = start; i < stack_.size(); i += step) {
      ...
      if (typed) {
        if (i == start) {
          ...
          // If you get this CHECK, you are writing a typed vector with
          // elements that are not all the same type.
          FLATBUFFERS_ASSERT(vector_type == stack_[i].type_);
        }
      }
    }
    // If you get this CHECK, your fixed types are not one of:
    // Int / UInt / Float / Key.
