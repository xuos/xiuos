...
uint32_t sysctl_clock_source_get_freq(sysctl_clock_source_t input)
{
    ...
    switch (input)
    {
        ...
        case SYSCTL_SOURCE_ACLK:
            result = SysctlClockGetFreq(SYSCTL_CLOCK_ACLK);
            break;
        default:
            result = 0;
            break;
...
uint32_t SysctlClockGetFreq(sysctl_clock_t clock)
{
    uint32_t source = 0;
    uint32_t result = 0;
    ...
    switch (clock)
    {
         ...
         */
        case SYSCTL_CLOCK_GPIO:
            source = SysctlClockGetFreq(SYSCTL_CLOCK_APB0);
            result = source;
            break;
        case SYSCTL_CLOCK_UART1:
            source = SysctlClockGetFreq(SYSCTL_CLOCK_APB0);
         ...
         */
        case SYSCTL_CLOCK_AES:
            source = SysctlClockGetFreq(SYSCTL_CLOCK_APB1);
            result = source;
            break;
        case SYSCTL_CLOCK_OTP:
            source = SysctlClockGetFreq(SYSCTL_CLOCK_APB1);
...
uint32_t SysctlPllSetFreq(sysctl_pll_t pll, uint32_t pll_freq)
{
    if(pll_freq == 0)
        return 0;
...
uint32_t sysctl_cpu_set_freq(uint32_t freq)
{
    ...
    cpu_freq = SysctlPllSetFreq(SYSCTL_PLL0, (sysctl->clk_sel0.aclk_divider_sel + 1) * 2 * freq);
    return cpu_freq;
}
...
uint64_t sysctl_get_time_us(void)
{
    ...
    return v_cycle * 1000000 / SysctlClockGetFreq(SYSCTL_CLOCK_CPU);
}
